...
def detect_default_branch() -> str:
    """The repository's default branch — not the branch a pull request targets.

    GITHUB_BASE_REF was consulted first and is the PR's *base*, so a PR into
    `staging` rewrote this field to `staging` while the repository default was
    `main`. That made the staleness check unsatisfiable: the committed value
    was correct, CI regenerated a different one, and writing CI's value in
    would have recorded something false and flipped back on the next PR
    targeting a different branch.

    VERITY_DEFAULT_BRANCH is supplied by the workflow from GitHub's own
    repository metadata, which is authoritative wherever it is available.
    """
    from_env = os.environ.get("VERITY_DEFAULT_BRANCH", "").strip()
    if from_env:
        return from_env
...
def existing_snapshot_value(content: str, label: str) -> str:
    prefix = f"- {label}: `"
...
def main() -> int:
    ...
    stable_snapshot = bool_value(os.environ.get("VERITY_DOCS_STABLE_SNAPSHOT"), False)
    context_commit_sha = existing_snapshot_value(context_content, "Commit") if stable_snapshot else ""
    context_commit_date = existing_snapshot_value(context_content, "Commit date") if stable_snapshot else ""
    handoff_commit_sha = existing_snapshot_value(handoff_content, "Commit") if stable_snapshot else ""
    handoff_commit_date = existing_snapshot_value(handoff_content, "Commit date") if stable_snapshot else ""
    ...
    context_snapshot = render_context_snapshot(
        commit_sha=context_commit_sha or commit_sha,
        commit_date=context_commit_date or commit_date,
    ...
    handoff_snapshot = render_handoff_block(
        commit_sha=handoff_commit_sha or commit_sha,
        commit_date=handoff_commit_date or commit_date,
        repository=repository,
        default_branch=default_branch,
        commands=commands,
    )
