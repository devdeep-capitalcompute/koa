...
def detect_tech_stack() -> dict:
    ...
    # --- AWS SAM / Lambda ---
    #
    # Repo-root only, originally. Verity's own repo is `backend/` + `frontend/`
    # and so is a great many others, and a template nested one directory down
    # was invisible to this check — the deterministic inventory read the
    # repository and reported no backend at all for one that has a working API.
    # `find_files` already walks the tree for package.json a few lines above;
    # this uses the same helper rather than a second, narrower check.
    if find_files(["template.yaml", "template.yml"]):
...
def extract_api_routes() -> list[dict]:
    ...
    #
    # Same fix as the stack detection above, for the same reason: a template
    # nested under `backend/` (or any subdirectory) was never read, so a
    # repository whose whole API is declared in SAM reported zero routes.
    for tmpl_path in find_files(["template.yaml", "template.yml"]):
